<x-form.guest>
    ...
    <h1 class="text-2xl font-bold text-center text-slate-800">Sign In</h1>

    <form action="{{ route('login.store') }}" method="post" class="p-10">
        @csrf
        ...
        <x-form.form-input id="password" type="password" name="password"
        ...
        <div class="text-sm text-slate-600 hover:text-slate-900 pt-4 text-center mt-5">
            <p>Dont have an account? <a href="{{ route('register') }}" class="text-greendark underline">Sign up</a></p>
        </div>
    </form>
</x-form.guest>